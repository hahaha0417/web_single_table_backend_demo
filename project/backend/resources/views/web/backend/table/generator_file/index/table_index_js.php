...
use hahaha\hahaha_generator_table_js_trait;
...
class table_index_js extends hahaha_generator_web_script
{
    ...
    use hahaha_generator_table_js_trait;
    ...
    */
    ...
    public function Class_Index_Item_Panel_Add_Button_Add(&$content, $dynamic = false, &$tab = "", &$tab_count = 0)
    {
        ...
        // ----------------------------------------
        \p_ha::Line($content, $tab, "// ---------------------------------------- ");
        \p_ha::Line($content, $tab, "// 項目太多，再另做一個功能(傳輸最簡) ");
        \p_ha::Line($content, $tab, "// 不然直接將form丟過去即可 ");
        \p_ha::Line($content, $tab, "// ---------------------------------------- ");
        // ----------------------------------------
        $data_ = [];
        $this->Button_Click_Table_Deal(
            $content,
            $dynamic,
            $tab,
            $tab_count,
            $use_->Id_Panel_Add_Button_Add_Identify,
            $use_->Id_Panel_Add_Form_Identify,
            //
            function() use(&$content, &$dynamic, &$tab, &$tab_count)
            {
                \p_ha::Line($content, $tab, "alert('進去了!!!!!!');");
            },
            $data_
        );

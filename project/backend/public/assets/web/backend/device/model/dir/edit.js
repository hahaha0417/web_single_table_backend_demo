...
$(function(){   
    ...
    // 
    var editor_title_content = CKEDITOR.replace('index_item_title_content', 
        {
            allowedContent : true,    
    ...
    editor_title_content.config.resize_enabled = false;
    editor_title_content.config.width = '800px';
    editor_title_content.config.height = '100px';
    ...
    var upload = window.location.protocol + "//" + $(location).attr('host') + "/backend/device/" + dir_ + "/dir/deal";
    // {
    ...
    // }   
    {
        index_item_image_upload = $("#index_item_image_upload").uploadFile({
            ...
            formData     : {
                ...
                'target': 'image',
                'item': {
                    "block": $("#unique_item").attr("unique_block"),
                    "item": $("#unique_item").attr("unique_item"),
                    "id": $("#unique_item").attr("unique_id"),
                },
            },
    ...
    // }  
    {   
        var block_auto_complete_tag_use = [];        
        $.each(block_auto_complete_tag, function( index, value ) { 
            block_auto_complete_tag_use.push(value['block']);
        }); 
        $("#index_item_block").autocomplete({
            source: block_auto_complete_tag_use
        });
    }
    {
    ...
    }
    {
        $("#index_item_image_refresh").click(function(){                         
            ...
            $.ajax({
                type:"POST",
                url:"/backend/device/" + dir_ + "/dir/deal",
                data:{
                    '_token': $("input[name=_token]").attr("value"),
                    'deal': 'item',
    ...
    {
        function index_item_block_change(this_){
            var id = $("#unique_item").attr("unique_id");
            var item = {
                "block": $("#index_item_block").val(),
            };
            ...
            $.ajax({
                ...
                success:function(response,status,xhr){  
                    ...
                    if(response.status == 0){
                        ...
                        $("#unique_item").attr("unique_block", $("#index_item_block").val());
        ...
        $("#index_item_block").change(function() {
            index_item_block_change(this);
        });
    }
                ...
                success:function(response,status,xhr){  
                    ...
                    if(response.status == 0){
                        ...
                        if(response.item['content']){
                            ...
                            items[item_item_index]['content'] = "";
                        }
                        if(response.item['comment']){
                            items[item_item_index]['comment'] = response.item['comment'];
                        }
                        else{
                            items[item_item_index]['comment'] = "";
                        }
                        ...
                        // alert(items[item_item_index]['content']);
                        if(items[item_item_index]['item'] != ""){
                            $("#index_item_item_select").append(
                                "<option no_=" + items[item_item_index]['no'] + " index=" + item_item_index + ">" + items[item_item_index]['order_'] + " - " + items[item_item_index]['item'] + "</option>"
                            ); 
...

    {
        $("#index_item_update").click(function(){
            var item = {               
                ...
                'title_content': editor_title_content.getData(),
                'image': $("#index_item_image").val(),

        ...
        ! function(t, e) {
            ...
            function c(e, n) {
                ...
                for (var S, A, T, C, O, k, N = new h(e); !N.eos();) {
                    ...
                    if (!N.scan(a)) break;
                    if (v = !0, A = N.scan(x) || "name", N.scan(y), "=" === A ? (T = N.scanUntil(E), N.scan(E), N.scanUntil(c)) : "{" === A ? (T = N.scanUntil(m), N.scan(w), N.scanUntil(c), A = "&") : T = N.scanUntil(c), !N.scan(c)) throw new Error("Unclosed tag at " + N.pos);
                    if (O = [A, T, S, N.pos], f.push(O), "#" === A || "^" === A) d.push(O);
                    else if ("/" === A) {
                        if (k = d.pop(), !k) throw new Error('Unopened section "' + T + '" at ' + S);

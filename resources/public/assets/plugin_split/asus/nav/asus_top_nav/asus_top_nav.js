...
(function() {
    ...
    function asus_menu() {
        ...
        var lock_show_over = function() {
            lock_show = true;
        };
        var lock_show_out = function() {
            lock_show = false;
        };
        ...
        var touch_menu = function() {
            ...
            if ((temp_select != select_nav) && (open) && (!asus.user.touch)) {
                return false;
            }
            // $("#asus-api-header .aai-subNav").removeClass('open');
            ...
            if (asus.user.touch) {
                ...
                open = false;
                autoheight(false, 1);
                if (!$('#asus-api-header .aai-focus').eq(0)[0]) {
                    $('#asus-api-header .js_line').stop().animate({
                        'width': 0,
                        'left': 0
                    });
                } else {
                    line($('#asus-api-header .aai-focus').eq(0));
                }
            }
        ...
        var autoheight = function(has, hide) {
            if (has) {
                ...
                $('.aai-subNav').css({
                    'height': $("#asus-api-header").height() + 'px',
                    'opacity': '0'
                }).stop().animate({
                    'height': height + 'px',
                    'opacity': '1'
                ...
                $('.aai-subNav').stop().animate({
                    'height': (hide == 1 ? $("#asus-api-header").height() : height) + 'px',
                    'opacity': (hide == 1 ? '0' : '1')
                }, 300, function() {
                    if (hide) {
                        $("#asus-api-header .aai-subNav").removeClass('open');
        ...
        var line = function(obj) {
            ...
            $('#asus-api-header .js_line').stop().animate({
                ...
                if (temp_select != obj) {
                    if ((temp_select) && (!select_nav)) { /*line(temp_select);*/ } else if (!select_nav) {
                        $('#asus-api-header .js_line').stop().animate({
                            'width': 0,
                            'left': 0
                        });
        ...
        var status = false;
        var run = function() {
            if (($(window).width() > 719) && (status == false)) {
                start();
                status = true;
            } else if (($(window).width() <= 719) && (status == true)) {
                clear_all();
                status = false;
            }
            if ($(window).width() > 719) {
    ...
    */
    asus.RecentlyView = {
        ...
        init: function() {
            ...
            asus.RecentlyView.init = function() {
                return;
            }
        },
        rehtml: function() {
            ...
            for (var intI = 0; intI < 10; intI++) {
                if (asus.RecentlyView.val[intI]) {
                    ...
                    $("#viewed-list ul li").eq(intI).html('<img src="https://www.asus.com/media/img/p_130_rull.jpg">');
                }
            }
            $("#viewed-list .removebutton").each(function(i) {
                $(this).attr("remove_i", i);
                $(this).on("click", function() {
                    asus.RecentlyView.reset($(this));
                    asus.RecentlyView.remove($(this).attr("remove_i"));
                    return false;
                });
            });
        ...
        recookie: function() {
            ...
            if (str) {
                asus.cookie.set("recently", str);
            } else {
                asus.cookie.del("recently");
            }
        },
        add: function(id, name, src) {
            ...
            if (count > 0) {
                for (var intI = 0; intI < count; intI++) {
                    if (asus.RecentlyView.val[intI][0] == id) {
                        return false;
                    }
                }
            }
    ...
    function apiheader() {
        var tag = false;
        var tag_id = $("#asus_top_nav");
        $("#asus_top_nav").addClass('poa');
        if (!tag_id[0]) {
            return;
        }
        ...
        var scroll_fn = function() {
            ...
            if (scrollTop >= offset_top) {
                ...
                if (offset_top == 0) {
                    offset_top = tag_id.offset().top;
                    return true;
                }
                tag = true;
                tag_id.css({
                    'top': 0 - tag_id.height() + 'px'
                }).addClass('fixed');
                tag_id.stop().animate({
                    'top': '0'
                });
                ...
                if ($(window).width() >= 752 && !$('body').hasClass('mobile')) {
                    tag_id.stop().animate({
                        'top': '0px'
                    });
                }
        ...
        if (window.siteConfig.product.PDIIP != undefined && window.siteConfig.product.PDIIP.ImgURL != undefined && window.siteConfig.product.PDIIP.ImgURL != '' && !$('body').hasClass('d-mobile')) {
            logoControl();
        }
    }
    ...
    function logoControl() {
        var logoTheme = window.siteConfig.product.PDIIP.BlackFontFlag != undefined ? window.siteConfig.product.PDIIP.BlackFontFlag == 0 ? true : false : false;
        var logoPosition = window.siteConfig.product.PDIIP.Position != 0 ? window.siteConfig.product.PDIIP.Position : false;
        var logoType = window.siteConfig.product.OverviewType;
        var logoUrl = window.siteConfig.product.PDIIP.ImgURL;
        var logoAlt = window.siteConfig.product.PDIIP.Desc1 ? window.siteConfig.product.PDIIP.Desc1 : 'brand logo';
        var logoDesc1 = window.siteConfig.product.PDIIP.Desc1 ? '<p class="desc" style="margin: 0px 8px;font-weight: 500;font-size: 14px;">' + window.siteConfig.product.PDIIP.Desc1 + '</p>' : '';
        var logoDesc2 = window.siteConfig.product.PDIIP.Desc2 ? '<p class="desc" style="margin: 0px 8px;font-weight: 500;font-size: 14px;">' + window.siteConfig.product.PDIIP.Desc2 + '</p>' : '';
        var logoDesc3 = window.siteConfig.product.PDIIP.Desc3 ? '<p class="desc" style="margin: 0px 8px;font-weight: 500;font-size: 14px;">' + window.siteConfig.product.PDIIP.Desc3 + '</p>' : '';
        var logoStyle = '';
        var right = '';
        var fontcolor = 'color: black;';
        var logo_html = '';

        /*detect for Black theme*/
        if (logoTheme) {
            fontcolor = 'color: white;';
        }

        var position_html = '<img src="' + logoUrl + '" alt=' + logoAlt + ' style="display: inline-block;vertical-align: top">' + '<div clas="desc-box" style="display: inline-block;vertical-align: bottom;text-align:left;' + fontcolor + '">' + logoDesc1 + logoDesc2 + logoDesc3 + '</div>';

        if (logoType == 1) { //normal layout
            if (logoPosition == 1 && logoPosition) {
                logoStyle = 'margin-top: 70px;left:0;';
                if ($('#asus_top_nav').hasClass('hasMDA')) {
                    logoStyle = 'margin-top: 115px;left:0;';
                }
            } else if (logoPosition == 2 && logoPosition) {
                logoStyle = 'margin-top: 70px;right:0;text-align: right;';
                if ($('#asus_top_nav').hasClass('hasMDA')) {
                    logoStyle = 'margin-top: 115px;right:0;text-align: right;';
                }

                position_html = '<div clas="desc-box" style="display: inline-block;vertical-align: bottom;text-align:left;' + fontcolor + '">' + logoDesc1 + logoDesc2 + logoDesc3 + '</div>' + '<img src="' + logoUrl + '" alt=' + logoAlt + ' style="display: inline-block;vertical-align: top">';
            } else if (logoPosition == 3 && logoPosition) {
                logoStyle = 'left: 0;margin-top:10px;';
            }

            logo_html = '<div id="brand_logo" style="position:relative;width:100%;z-index:999;' + logoStyle + '">' + position_html + '</div>';

            if (logoPosition == 3 && logoPosition) {
                $('#overview').css('margin-top', '50px');
                $('#product_content_area').css('position', 'relative').find('#overview').after(logo_html);
            } else {
                $('#overview').css('margin-top', '0px');
                $('#product_content_area').css('position', 'relative').find('#overview').before(logo_html);
            }
        } else if (logoType == 2 || logoType == 4) {
            if (logoPosition == 1 && logoPosition) {
                if (logoType == 2) {
                    logoStyle = 'top: 62px;left:0;right:0;';
                } else {
                    logoStyle = 'top: 50px;left:0;right:0;';
                }
            } else if (logoPosition == 2 && logoPosition) {
                if (logoType == 2) {
                    logoStyle = 'top: 62px;left:0;right:0;text-align: right;';
                } else {
                    logoStyle = 'top: 85px;left:0;right:0;text-align: right;';
                }

                position_html = '<div clas="desc-box" style="display: inline-block;vertical-align: bottom;text-align:left;' + fontcolor + '">' + logoDesc1 + logoDesc2 + logoDesc3 + '</div>' + '<img src="' + logoUrl + '" alt=' + logoAlt + ' style="display: inline-block;vertical-align: top">';
            } else if (logoPosition == 3 && logoPosition) {
                logoStyle = 'bottom: 0;left: 0;right:0;';
            }

            logo_html = '<div id="brand_logo" style="position:absolute;max-width:1170px;width:100%;margin:auto;z-index:999;' + logoStyle + '">' + position_html + '</div>';

            $('#product_content_area').css('position', 'relative').append(logo_html);
        } else {
            if (logoPosition == 1 && logoPosition) {
                if (logoType == 2) {
                    logoStyle = 'top: 62px;left:0;';
                } else {
                    logoStyle = 'top: 50px;left:0;';
                }
            } else if (logoPosition == 2 && logoPosition) {
                if (logoType == 2) {
                    logoStyle = 'top: 62px;right:0;text-align: right;';
                } else {
                    logoStyle = 'top: 85px;right:0;text-align: right;';
                }

                position_html = '<div clas="desc-box" style="display: inline-block;vertical-align: bottom;text-align:left;' + fontcolor + '">' + logoDesc1 + logoDesc2 + logoDesc3 + '</div>' + '<img src="' + logoUrl + '" alt=' + logoAlt + ' style="display: inline-block;vertical-align: top">';
            } else if (logoPosition == 3 && logoPosition) {
                logoStyle = 'bottom: 0;left: 0;';
            }

            logo_html = '<div id="brand_logo" style="position:absolute;width:100%;z-index: 999;' + logoStyle + '">' + position_html + '</div>';
    ...
    function aai_right_more() {
        ...
        var aairightmore_open = function() {
            ...
            if (obj.find('.aai-mm-item').hasClass('open')) {
                ...
                $('body').off('mousedown', out_click);
            }

            var link_id = $(this).data('id');
            var link_url = $(this).data('url');
            obj.find('.aai-more-tools').addClass('exp');
            obj.find('.aai-more-tools').animate({
                'width': $(".aai-inner").width() + 'px'
            });
            if (($('.aai-more-tools').hasClass('open')) || ($(window).width() <= 719)) {
                obj.find('.aai-more-inner').css({
                    'width': ($(".aai-inner").width() - parseInt(obj.find('.aai-more-inner').css('padding-left')) - parseInt(obj.find('.aai-more-inner').css('padding-right'))) + 'px'
                });
            }
            if ($('.aai-more-tools').hasClass('exp')) {
                obj.find('.aai-more-tools').css({
                    'width': $(".aai-inner").width() + 'px'
                });
            }
            if (link_url) {
        ...
        var resetMessage = function() {
            var msg_data = (asus.script.get_local() == "cn") ? '//www.asus.com.cn/message.aspx?lang=' + asus.script.get_local() :
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res.data == "0") $("#menu_msg .aai-circle-num").css({
                        'display': 'none'
                    });
                    else $("#menu_msg .aai-circle-num").css({
                        'display': 'block'
                    });
                },
                error: function() {}
        ...
        obj.find('.aai-mm-sub,.aai-mm-btn').on("mouseenter", function() {
            in_tool = true;
        });
        obj.find('.aai-mm-sub,.aai-mm-btn').on("mouseleave", function() {
            in_tool = false;
        });
    ...
    function phone_menu() {
        ...
        var sub2 = function() {
            ...
            if (($(this).find('a').attr('href').search("#") == -1) && ($(this).find('a').attr('href').search("javascript") == -1)) {
                return true;
            }
    ...
    function overviewnav() {
        var show = function() {
            $('#asus_top_nav').toggleClass('open');
        };
        var start = function() {
            $("#asus_top_nav h1").on("click", show);
        };
        var status = false;
        var clear_all = function() {
            $("#asus_top_nav h1").off("click", show);
        };
        var run = function() {
            if (($(window).width() > 769) && (status == true)) {
                clear_all();
                status = false;
            } else if (($(window).width() <= 769) && (status == false)) {
                start();
                status = true;
            }
        };
    ...
    // overview top nav
    function Androidapp() {
        if (asus.cookie.get("passAndroid") || (in_page != 'Index')) {
            return false;
        }
                    ...
                    success: function(xml) {
                        ...
                        if ($("title", obj_tip).text() != "") {
                            ...
                            if (getStorage('asus-version') == 'desktop') {
                                ...
                                $('#asus_top_nav').css({
                                    'top': ($('#asus-api-header').offset().top + 89) + 'px'
                                });
                            }

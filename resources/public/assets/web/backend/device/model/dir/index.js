...
$(function(){   
    ...
    function upload_file(upload, item, index){
        item_image_upload_file[index] = $(item).uploadFile({
            ...
            formData     : {
                ...
                'target': 'image',
                'item': {
                    "block": $(item).parent().parent().parent().find(".index_item_panel_block").val(),
    ...
    function item_add_event_setting(this_, index){
        ...
        var upload = window.location.protocol + "//" + $(location).attr('host') + $(location).attr('pathname') + "/deal";
        ...
        $("#index_item_name_prepend_" + index).mouseenter(function(){
            var panel = $(this).parent().find(".index_item_panel");
            index_item_name_prepend_mouseenter(this, panel);            
        });
        $("#index_item_name_prepend_" + index).mouseleave(function(e){
            index_item_name_prepend_mouseleave(this, e);
        });
        ...
        $("#index_item_panel_block_" + index).autocomplete({
            source: block_auto_complete_tag
        });

        $("#index_item_panel_block" + index).change(function() {
            index_item_panel_block_change(this);
        });

        $("#index_item_panel_item_" + index).autocomplete({
            source: item_auto_complete_tag
        });
        ...
        <tr id=\"index_item_" + index + "\" class=\"index_item\" item_id=\"" + item['id'] + "\" index=\"" + index + "\">                             \
            ...
            <td>                \
                <div class=\"input-group\">                 \
                    <input id=\"index_item_name_" + index + "\" type=\"text\" class=\"index_item_name form-control\" placeholder=\"名稱\" value=\"" + item['name'] + "\">              \
                    <div id=\"index_item_name_prepend_" + index + "\" class=\"input-group-prepend index_item_name_prepend\">                 \
                    ...
                    <div id=\"index_item_panel_" + index + "\" class=\"index_item_panel\">                   \
                        <div class=\"index_item_panel_content\" style=\"overflow-y:scroll;\">                   \
                            <div class=\"row\">                     \
                                <label for=\"index_item_panel_block_" + index + "\" class=\"col-sm-3 col-form-label\">區塊 : </label>                \
                                <div class=\"col\">                 \
                                    <input style=\"width:530px;\" id=\"index_item_panel_block_" + index + "\" style=\"\" type=\"text\" class=\"index_item_panel_block form-control\" placeholder=\"區塊\" value=\"" + item['block'] + "\">         \
                                </div>                      \
                            </div>                      \
            ...
            <td>                        \
                <div class=\"\" >                                   \
                    <img id=\"index_item_image_thumbnail_" + index + "\" class=\"index_item_image_thumbnail\" src=\"0\" style=\"width: 40px;height: 40px;\"/>                        \
                    <div id=\"index_item_image_upload_" + index + "\" class=\"index_item_image_upload\" style=\"margin-right:15px;display:inline-block;\" name=\"index_item_image_upload_0\" type=\"file\"></div>        \
                </div>                  \
            </td>                           \
            <td>                            \
                <div class=\"\" >                       \
    ...
    function add_item(){
        ...
        var item = {
            "block": $("#index_item_add_panel_block").val(), 
            "item": $("#index_item_add_panel_item").val(), 
            "id": $("#index_item_add_panel_id").val(), 
            "name": $("#index_item_add_panel_name").val(), 
        };
        ...
        $.ajax({
            ...
            success:function(response,status,xhr){  
                ...
                if(response.status == 0){
                    ...
                    if(response.item["image"] == null){
                        response.item["image"] = "";
                    }
                    if(response.item["url"] == null){
                        response.item["url"] = "";
                    }
    ...
    $(".index_item_add").click(function() {        
        ...
        $("#index_item_add_panel_block").val("");
        $("#index_item_add_panel_item").val("");
        $("#index_item_add_panel_id").val("");
        $("#index_item_add_panel_name").val("");
    ...
    function item_all_save(){
        ...
        $.each($(".index_item"), function( index, value ) {  
            item.push(
                {
                    "block": $(this).find(".index_item_page").val(),
                    "item": $(this).find(".index_item_item").val(),
                    "order_": $(this).find(".index_item_order").val(),
                    "id": $(this).attr("item_id"),
                    "name": $(this).find(".index_item_name").val(),
    ...
    function index_item_panel_block_change(this_){
        // alert($(this_).parent().parent().parent().parent().parent().parent().parent().find(".index_item_panel_block").val());
        var id = $(this_).parent().parent().parent().parent().parent().parent().parent().attr("item_id");
        var item = {
            "block": $(this_).parent().parent().parent().parent().parent().parent().parent().find(".index_item_panel_block").val(),
            "item": $(this_).parent().parent().parent().parent().parent().parent().parent().find(".index_item_panel_item").val(),
        };
    ...
    $(".index_item .index_item_panel_block").change(function() {        
        index_item_panel_block_change(this);
    });
    ...
    
    {
        ...
        var item_auto_complete_tag = [];    
        var block_auto_complete_tag = [];      
        
        $.each(block_list_, function( index, value ) { 
            block_auto_complete_tag.push(value['block']);
        });
        $.each(item_list_, function( index, value ) { 
            item_auto_complete_tag.push(value['item']);
        });

        $.each($(".index_item_panel_block"), function( index, value ) {  
            $( value ).autocomplete({
                source: block_auto_complete_tag
            });
        });
    ...
    function index_item_name_prepend_mouseenter(this_, panel){
        if(is_in_iframe){
            if($(this_).offset().top - $(window).scrollTop() - $(panel).height() >= 0){
                panel.css("left", $(this_).offset().left - $(window).scrollLeft() - ($(panel).width() - $(this_).width()) / 2 + 100) + "px";   
    ...
    function index_item_name_prepend_mouseleave(this_, event){
    ...
    {
        $(".index_item .index_item_name_prepend").mouseenter(function(){            
            var panel = $(this).parent().find(".index_item_panel");
            index_item_name_prepend_mouseenter(this, panel);            
        });
        $(".index_item .index_item_name_prepend").mouseleave(function(event){
            index_item_name_prepend_mouseleave(this, event);
        });
        var panel_ok = false;

...
(function(){
  ...
  function showTopBar(_string, _yes, _no){
    ...
    if (asus.cookie.get('isReadCookiePolicyDNT') != '' && asus.cookie.get('isReadCookiePolicyDNT') != undefined){
      ...
      var newDom = "<div id='cookie-policy-info'>"
      + "<div class='wrap' style='box-sizing: initial;font-size:14px;background-color: #424242;'>" + _string + "<br>"
      + "<a id='btn-read-ck' class='btn-asus btn-ok' style='box-sizing: initial;'>" + _yes + "</a>"
      + "<a id='btn-noread-ck' class='btn-asus btn-reject' style='box-sizing: initial;'>" + _no + "</a>"
      + "</div><span id='btn-close-ck' class='close'>x</span></div>";

      $(newDom).prependTo("body");
      $("body").addClass('show-cookie-policy-info').css({'overflow-x': 'visible','top': '220px','font-family':'"Segoe UI", "微軟正黑體", "Microsoft JhengHei", "Arial", "新細明體"'});
      ...
      $("#cookie-policy-info")
        .find(".btn-ok")
          .on("click", function(){
            ...
            $("body").removeClass('show-cookie-policy-info').css({'overflow-x':'hidden', 'top': 'initial'});
